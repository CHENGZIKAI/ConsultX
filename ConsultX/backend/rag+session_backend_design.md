...
```
┌────────────────┐        ┌─────────────────────┐         ┌─────────────────────┐
│  REST API      │  HTTP  │  SessionTracker     │   SQL   │  Persistence        │
│ (backend/api)  ├───────▶│ + Sentiment & Risk  ├────────▶│ (SQLite via storage)│
└────────────────┘        ├─────────────────────┤         └─────────────────────┘
         ▲                │  Buffer Cache       │
         │                └─────────┬───────────┘
         │                          │
...
- `orchestrator.run_generate_reply()`: main entry point.
- `risk_types.py`: advanced risk assessment (tier, score, dimensions).
- `retrieval.py`: vector store + MI/CBT context retrieval.
- `prompt.py`: MI/CBT system + example prompts.
- `guardrails.py`: crisis overrides, MI-style softening, MI tags.
- `llm_gateway.py`: Gemini/OpenAI wrapper (API keys via env).
- `memory.py`, `session_store.py`: in-memory history summarization.
