...
# -------------------- MODEL WRAPPERS --------------------
def _build_pipeline(task: str, model: str, **kwargs):
    if not hf_pipeline:
        return None
    try:
...
_emotion_pipe = _build_pipeline("text-classification", model=HF_EMOTION_MODEL, return_all_scores=True)
_zsl_pipe = _build_pipeline("zero-shot-classification", model=ZSL_MODEL)
